...
function create(tag, attrs, html) {
  ...
  if (attrs) for (attrName in attrs) {
    ...
    } else if (attrs[attrName]) ele.setAttribute(attrName, attrs[attrName]);
  }
...
function getSettings() {
  notlikedbtn.classList.toggle("btn-active", notLiked);
  resultbtn.classList.toggle("btn-active", onMainDiv);
  hideuserbtn.classList.toggle("btn-active", hideUser);
  let activitiesidarray = window.localStorage.getItem('blockarray');
  if (activitiesidarray !== null) {
    let ars = activitiesidarray.split(/[.,!,?]/);
    blacklistarray = ars;
  }
  let listarray = window.localStorage.getItem('liststatus');
  if (listarray === null || listarray === '') {
    listarray = [];
  } else {
    let ars2 = listarray.split(/[.,!,?]/);
    liststatus = ars2;
  }
  for (i = 0; i < 3; i++) {
    if (liststatus.indexOf('CURRENT') === -1) {
      button10.classList.toggle("btn-active", false);
    } else {
      button10.classList.toggle("btn-active", true);
    }
    if (liststatus.indexOf('COMPLETED') === -1) {
      button9.classList.toggle("btn-active", false);
    } else {
      button9.classList.toggle("btn-active", true);
    }
    if (liststatus.indexOf('PLANNING') === -1) {
      button11.classList.toggle("btn-active", false);
    } else {
      button11.classList.toggle("btn-active", true);
    }
  }
}
let switchbutton = create("button", {
  class: "mainbtns",
  id: "switchbtn"
}, '<p>' + "Anime" + '</p>' + "/" + '<p>' + "Manga" + '</p>');
...
button3.onclick = () => {
  clearInterval(interval), filterAll();
};
...
let button6 = create("button", {
  class: "mainbtns",
  id: "resultbtn"
}, "Move Results");
button6.onclick = () => {
  onMainDivCheck(), $(".feed-type-toggle > div.active").click();
};
let button7 = create("button", {
  class: "mainbtns",
  id: "notlikedbtn"
}, "Not Liked");
button7.onclick = () => {
  notLikedCheck(), $(".feed-type-toggle > div.active").click();
};
let button8 = create("button", {
  class: "mainbtns",
  id: "hideuserbtn"
}, "Hide " + username);
button8.onclick = () => {
  hideUserCheck(), $(".feed-type-toggle > div.active").click();
};
let button9 = create("button", {
  class: "mainbtns",
  id: "currentbtn"
}, "Completed ");
button9.onclick = () => {
  liststatus.indexOf("COMPLETED") === -1 ? liststatus.push("COMPLETED") : liststatus.splice(liststatus.indexOf("COMPLETED"), 1);
  localStorage.setItem("liststatus", [liststatus]);
  getSettings();
  getlist();
};
let button10 = create("button", {
  class: "mainbtns",
  id: "completedbtn"
}, "Current ");
button10.onclick = () => {
  liststatus.indexOf("CURRENT") === -1 ? liststatus.push("CURRENT") : liststatus.splice(liststatus.indexOf("CURRENT"), 1);
  localStorage.setItem("liststatus", [liststatus]);
  getSettings();
  getlist();
};
let button11 = create("button", {
  class: "mainbtns",
  id: "planningbtn"
}, "Planning ");
button11.onclick = () => {
  liststatus.indexOf("PLANNING") === -1 ? liststatus.push("PLANNING") : liststatus.splice(liststatus.indexOf("PLANNING"), 1);
  localStorage.setItem("liststatus", [liststatus]);
  getSettings();
  getlist();
};
let button12 = create("button", {
  class: "mainbtns",
  id: "blacklistbtn",
  style: {
    position: "absolute",
...
start();
function start() {
  if (!/^\/(home|user)\/?([\w-]+)?\/?$/.test(location.pathname)) {
    return;
  }
...
function createDiv() {
  ...
  if (active) {
    ...
    let listDiv = create("div", {
      class: "maindiv",
      id: "listDiv"
    }, '<div class="maindivheader"><b>' + button.innerText + '</b></div>');
    ...
    let animedataDiv = create("div", {
      class: "animedataDiv",
      id: "animedataDiv"
    });
    listDiv.appendChild(animedataDiv);
    let ResultDiv = create("div", {
      class: "maindiv",
      id: "ResultDiv",
      style: {
        display: "none",
    ...
    let ResultDivInside = create("div", {
...
function filterAll() {
  ...
  if (elem.length > 0) {
    ...
    window.alert("Error:The list is empty. Please add anime or manga.");
  }
}
...
function getlist() {
  ...
  animedataDiv.appendChild(maindiv);
  liststatus.forEach(function (data) {
    listcurrent = data;
    listquery();
  });
  function listquery() {
    let title = create("div", false, "<b>" + listcurrent + "</b>");
    maindiv.appendChild(title);
    if (liststatus.length === 1) {
      title.children[0].style.cssText = "display:none";
    }
      ...
      data.data.MediaListCollection.lists.forEach(list => {
        list.entries.forEach(animedata => {
          ...
          if (findDuplicates(strArray).length < 1) {
            let aimg = create("a", {
              class: "animedataimg",
              href: animedata.media.siteUrl,
              style: {
                backgroundImage: "url(" + animedata.media.coverImage.large + ")"
              }
            });
            let a = create("a", {
              class: "animedata",
              id: animedata.media.id
            });
            let alink = create("a", {
              class: "animedatalink",
              id: animedata.media.id
  ...
  following(userId: $id, sort:USERNAME) {name avatar{medium} siteUrl}}}`;
    ...
    function handleData(data) {
      data.data.Page.following.forEach(following => {
        let userimg = create("a", {
          class: "userdataimg",
          href: following.siteUrl,
          style: {
            backgroundImage: "url(" + following.avatar.medium + ")"
          }
        });
        let userdata = create("a", {
          class: "userdata"
        });
...
function search() {
  ...
  function handleData(data) {
    ...
    data.data.Page.media.forEach(animedata => {
      let aimg = create("a", {
        class: "animedataimg",
        href: animedata.siteUrl,
        style: {
          backgroundImage: "url(" + animedata.coverImage.large + ")"
        }
      });
      let a = create("a", {
        class: "animedata",
...
function blacklistcheck() {
  ...
  userdata.forEach(userinfo => {
    if (blacklistarray.indexOf(userinfo.innerText) === -1) {
      ...
      userinfo.classList.toggle("blocked", true);
    }
  });
}
function blacklistclick() {
  each('.userdata', function (el) {
    ...
    el.onclick = () => {
      ...
      let activitiesidarray = window.localStorage.getItem('blockarray');
      let x = activitiesidarray.split(/[.,!,?]/);
      localStorage.setItem("blockarray", x);
      blacklistcheck();
    };
  });
  function each(el, callback) {
